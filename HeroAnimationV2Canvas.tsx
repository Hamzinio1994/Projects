// ─────────────────────────────────────────────────────────────────────────────
// HeroAnimationV2Canvas.tsx
//
// PURE component — no Framer-specific imports.
// This file is hosted on GitHub and consumed by the thin Framer wrapper via:
//   https://esm.sh/gh/YOUR_USER/YOUR_REPO@main/src/app/pages/HeroAnimationV2Canvas.tsx
//     ?bundle&external=react,framer-motion
//
// To update Framer: edit this file → push to GitHub.
// The wrapper in Framer never needs to change.
// ─────────────────────────────────────────────────────────────────────────────
...
export type SizeKey = "desktop" | "tablet" | "mobile"

// ─── LAYOUT CONFIGS ───────────────────────────────────────────────────────────
const CFGS: Record<SizeKey, LayoutCfg> = {
    desktop: {
        canvas: { w: 960, h: 600 },
        unit: {
            left: 105, top: 64,
...
function ViewfinderMetaHUD({ worker, sW, sH }: { worker: typeof WORKERS[number]; sW: number; sH: number }) {
    ...
    return (
        <div style={{ position: "absolute", top, left: "50%", transform: "translateX(-50%)", pointerEvents: "none", zIndex: 22 }}>
            <div style={{
                ...
                <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-start" }}>
                    <span style={{ fontSize: fs, fontWeight: 700, color: "#fff", lineHeight: 1.2 }}>{worker.location}</span>
                    {sW >= 90 && (
                        <span style={{ fontSize: fsS, color: "rgba(255,255,255,0.58)", lineHeight: 1.2 }}>{worker.sublocation}</span>
                    )}
                </div>
            </div>
        </div>
...
function PhotoStamp({ worker, sW, sH, doAnimate = true }: {
    ...
    const bar = (
        <div style={{
            ...
            padding: `${vpad}px ${pad}px`, gap: rowGap, pointerEvents: "none",
        }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: pinGap }}>
                ...
                <span style={{ fontSize: fs, fontWeight: 700, color: "#fff", whiteSpace: "nowrap", letterSpacing: "-0.01em" }}>{worker.location}</span>
            </div>
            <div style={{ display: "flex", justifyContent: "center" }}>
                <span style={{ fontSize: fsT, fontWeight: 500, color: "rgba(255,255,255,0.46)", whiteSpace: "nowrap", fontVariantNumeric: "tabular-nums" }}>
                    {worker.time} · 03 Mar
                </span>
            </div>
        </div>
...
function GridMetaPill({ worker, headerH }: { worker: typeof WORKERS[number]; headerH: number }) {
    ...
    return (
        <motion.div
            ...
            style={{
                position: "absolute",
                bottom: Math.round(headerH * 0.12), right: Math.round(headerH * 0.1),
...
function IPhoneBody({ phoneW, phoneH, worker, children, isShutter = false }: {
    ...
    const bz  = Math.round(phoneW * 0.04)
    const sW  = phoneW - bz * 2
    const cr  = Math.round(phoneW * 0.20)
    const sBR = Math.round(phoneW * 0.165)
    ...
    return (
        <div style={{
            ...
            background: "#0A0E1A", borderRadius: cr,
            ...
            <div style={{
                ...
                height: phoneH - bz * 3,
                borderRadius: sBR, overflow: "hidden", background: "#050A14",
            }}>
                {children}
                <div style={{
                    position: "absolute", top: diTop, left: "50%",
                    transform: "translateX(-50%)",
                    width: diW, height: diH, borderRadius: diH,
        ...
        <AnimatePresence>
            {visible && (
                ...
                >
                    ...
                    <div style={{ position: "absolute", top: -unit.cardH, left: 0, zIndex: 55 }}>
            ...
            <div style={{ display: "flex", height: app.showChrome ? `calc(100% - ${app.chromeH}px)` : "100%", background: "#fff" }}>
                ...
                <div style={{ flex: 1, display: "flex", flexDirection: "column", position: "relative", overflow: "hidden", background: "#fff" }}>
                    ...
                    <div style={{
                        ...
                        <span style={{
                            fontSize: Math.round(app.headerH * 0.27), fontWeight: 600,
                            color: "#94A3B8", textTransform: "uppercase" as const, letterSpacing: "0.07em",
                        }}>Today</span>
                    </div>
...
// ─── CANVAS (default export) ──────────────────────────────────────────────────
export default function HeroAnimationV2Canvas({ size }: { size: SizeKey }) {
    const cfg   = CFGS[size]
